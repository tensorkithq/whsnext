...
**This plan delivers:** Track 3, part B. The Celeris script engine (README §5 JSON contract over `Whn.Fal.vision`) and `Whn.Pipeline` implementing the pinned message contract: celeris → bridge → last frame → 3 chained 10s segments, winner-only, with retries and no vote reopening.

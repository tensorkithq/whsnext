...
- **D-05 (Celeris model):** Celeris = fal `openrouter/router/vision` with `model: "google/gemini-2.5-flash-lite"`. The latest extracted frame is **always attached** in `image_urls` when one exists (grounds the script and sidesteps the unverified text-only-call behavior). *(session-locked)*

defmodule Whn.Prompts do
  ...
  - Sound: ambient environmental audio and cinematic score, with expressive but wordless vocal reactions — laughs, gasps, exclamations, grunts — layered over them. Any voices are wordless: no spoken dialogue ever, in any language. Both bridge and scene carry reactions, ambience, and score only.
  ...
  # The show is wordless. Enforced mechanically, not just requested:
  # the model's output is untrusted like every other field.
  @quoted ~r/"[^"]+"/

  @doc """
  Removes every quoted dialogue span. Applied to both bridge and scene
  video_prompts (and again per segment) so no spoken line ever reaches
  the video model.
  """

defmodule Whn.PipelineTest do
  ...
  describe "VIDEO_ENGINE=omni" do
    ...
    test "opening fans out from the flux frame; only the scene-end frame extracts" do
      {:ok, _pid} = Whn.Pipeline.start_opening(self(), ctx(%{beat: 0, winning_choice: nil}))
      ...
      # scene-end chaining still runs under omni: exactly one extraction upload
      assert_receive {:pipeline, 0, {:last_frame, frame_url}}, 10_000
      assert frame_url =~ "mock://frame-"
      assert [_one] = for({:upload, args} <- Whn.FalMock.calls(), do: args)
    end
  end
